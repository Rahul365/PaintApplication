...
namespace DLOGS {
    ...
    // This code snippet demonstrates how to work with the common file dialog interface
     HRESULT BasicFileOpen(std::string& filePath, bool saveFilePath)
    {
                                    ...
                                    if (SUCCEEDED(hr))
                                    {
                                        ...
                                        if (SUCCEEDED(hr))
                                        {
                                            ...
                                            if (SUCCEEDED(hr))
                                            {
                                                ...
                                                if (*pszFilePath) {
                                                    for (size_t index = 0; *(pszFilePath + index); ++index) {
                                                        filePath += *(pszFilePath + index);
                                                    }
                                                    OutputDebugStringA("Basic file open : ");
     ...
     HRESULT WritePropertiesWithoutUsingHandlers(std::string dataToWrite)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            const COMDLG_FILTERSPEC rgSaveTypes[] = { {L"MyApp Document (*.pnt)", L"*.pnt"} };
            ...
            if (SUCCEEDED(hr))
            {
                ...
                {
                    // Set default file extension.
                    hr = pfsd->SetDefaultExtension(L"pnt");
                    if (SUCCEEDED(hr))
                    {
                        // Ensure the dialog only returns items that can be represented by file system paths.
                        DWORD dwFlags;
            ...
            if (SUCCEEDED(hr))
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    if (SUCCEEDED(hr))
                    {
                        ...
                        hr = psiResult->GetDisplayName(SIGDN_FILESYSPATH, &pszNewFileName);
                        std::string path;
                        for (size_t index = 0; *(pszNewFileName + index); index++) {
                            path += *(pszNewFileName + index);
                        }

                        OutputDebugStringA("SAVE FILE PATH : ");
                        OutputDebugStringA(path.c_str());
                        OutputDebugStringA("\n");
                        if (SUCCEEDED(hr))
                        {
                            // Write data to the file.
                            hr = writeDataToFileUsingFSTREAM(path, dataToWrite);
